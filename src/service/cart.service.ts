...
export async function removeUserCart() {
  try {
    ...
    return {
      data: data,
      success: true,
      message: data.message || "Removing cart successfully",
    };
  } catch (error) {
    return {
      data: null,
...
export async function removeSpecificItem(productId : string) {
  try {
    const token = await getUserToken();
  ...
  try {
    ...
    if (!response.ok) {
      return {
        data: null,
        success: false,
        message: data.message || "Updating quantity cart failed ",
      };
    }
    return {
      data: data,
